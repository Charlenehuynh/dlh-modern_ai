...
for msg in df[
    (df["label"] == "spam") & df["message"].str.contains("http|www", regex=True)
]["message"].iloc[:1]:
    print(f"before : {msg[:100]}")
    print(f"after  : {clean_text(msg)[:100]}\n")

for msg in df[
    (df["label"] == "spam") & df["message"].str.contains(r"\d{8,}", regex=True)
]["message"].iloc[:2]:
    print(f"before : {msg[:100]}")
    print(f"after  : {clean_text(msg)[:100]}\n")

print(
    "Phone patterns matched first to avoid partial replacement by the general digit pass.\n"
)
