...
train_with_gradient_descent_variant = __import__(
    "0-gradient_descent_variants"
).train_with_gradient_descent_variant
...
for variant in ["batch", "mini_batch", "stochastic"]:
    ...
    history = model.fit(
        x_train, y_train, batch_size=bs, epochs=epochs, validation_split=0.2, verbose=1
    )
    end_time = time.time()
...
df = pd.DataFrame(
    results, columns=["Variant", "Val Acc", "Train Acc", "Train Time (s)"]
)
